...
<%
	int idMusic = 0, level = 0, idChallenge = 0;
	
	int idMuiscLv = 0;
	if(request.getParameter("idLv") != null){
		idMuiscLv = Integer.parseInt(request.getParameter("idLv"));
		MusicLevel lv = MusicLevelDAO.finByID(idMuiscLv);
		idMusic = lv.getIdMusic();
		level = lv.getLevel();
	}
	
	if(idMuiscLv == 0) {
		try {
			idMusic = Integer.parseInt(request.getParameter("id"));
	...
	Music music = MusicDAO.getMusicByID(idMusic);
%>
...
<script>
	...
	function initLoader() {
		...
		addMusicInfoUsingAudioTag('<%=music.getFileurl()%>');
	...
	function getNoteData() {
		...
		//Lay du lieu 
		//Get data from sever
		$.getJSON( "MusicLoader", { id : <%= idMusic%>, lv :  <%= level%>} )
		.done(function( data ) {
	...
	function endMusic(){
		...
		<% if(idChallenge == 0) { %>
			$("#score").load("score.jsp?score="+score);
		<%} else {%>
			$("#score").load("score.jsp?score="+score+"&challenge=<%= idChallenge%>");
		<%}%>
